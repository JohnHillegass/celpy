...
# python wrapper for package go within overall package ext
...
class Slice_bool(GoClass):
	...
	def __init__(self, *args, **kwargs):
		...
		if len(kwargs) == 1 and 'handle' in kwargs:
			...
			_ext.IncRef(self.handle)
		elif len(args) == 1 and isinstance(args[0], GoClass):
			self.handle = args[0].handle
			_ext.IncRef(self.handle)
		else:
			self.handle = _ext.Slice_bool_CTor()
			_ext.IncRef(self.handle)
			if len(args) > 0:
	...
	def __len__(self):
		return _ext.Slice_bool_len(self.handle)
	def __getitem__(self, key):
		if isinstance(key, slice):
			...
			return _ext.Slice_bool_elem(self.handle, key)
		else:
	...
	def __setitem__(self, idx, value):
		if idx < 0:
			idx += len(self)
		if idx < len(self):
			_ext.Slice_bool_set(self.handle, idx, value)
			return
		raise IndexError('slice index out of range')
	...
	def __next__(self):
		if self.index < len(self):
			rv = _ext.Slice_bool_elem(self.handle, self.index)
			self.index = self.index + 1
			return rv
		raise StopIteration
	def append(self, value):
		_ext.Slice_bool_append(self.handle, value)
	def copy(self, src):
...
class Slice_byte(GoClass):
	...
	def __init__(self, *args, **kwargs):
		...
		if len(kwargs) == 1 and 'handle' in kwargs:
			...
			self.handle = _ext.Slice_byte_CTor()
			_ext.IncRef(self.handle)
			if len(args) > 0:
				if not isinstance(args[0], collections.Iterable):
	...
	def __len__(self):
		return _ext.Slice_byte_len(self.handle)
	def __getitem__(self, key):
		if isinstance(key, slice):
			return [self[ii] for ii in range(*key.indices(len(self)))]
		elif isinstance(key, int):
			...
			if key < 0 or key >= len(self):
				raise IndexError('slice index out of range')
			return _ext.Slice_byte_elem(self.handle, key)
		else:
			raise TypeError('slice index invalid type')
	def __setitem__(self, idx, value):
		if idx < 0:
			idx += len(self)
		if idx < len(self):
			_ext.Slice_byte_set(self.handle, idx, value)
			return
		raise IndexError('slice index out of range')
	def __iadd__(self, value):
		if not isinstance(value, collections.Iterable):
	...
	def __next__(self):
		if self.index < len(self):
			rv = _ext.Slice_byte_elem(self.handle, self.index)
			self.index = self.index + 1
			return rv
		raise StopIteration
	def append(self, value):
		_ext.Slice_byte_append(self.handle, value)
	def copy(self, src):
		""" copy emulates the go copy function, copying elements into this list from source list, up to min of size of each list """
		mx = min(len(self), len(src))
		for i in range(mx):
...
class Slice_float32(GoClass):
	...
	def __init__(self, *args, **kwargs):
		...
		if len(kwargs) == 1 and 'handle' in kwargs:
			...
			self.handle = _ext.Slice_float32_CTor()
			_ext.IncRef(self.handle)
			if len(args) > 0:
				if not isinstance(args[0], collections.Iterable):
	...
	def __len__(self):
		return _ext.Slice_float32_len(self.handle)
	def __getitem__(self, key):
		if isinstance(key, slice):
			return [self[ii] for ii in range(*key.indices(len(self)))]
		elif isinstance(key, int):
			...
			if key < 0 or key >= len(self):
				raise IndexError('slice index out of range')
			return _ext.Slice_float32_elem(self.handle, key)
		else:
			raise TypeError('slice index invalid type')
	def __setitem__(self, idx, value):
		if idx < 0:
			idx += len(self)
		if idx < len(self):
			_ext.Slice_float32_set(self.handle, idx, value)
			return
		raise IndexError('slice index out of range')
	def __iadd__(self, value):
		if not isinstance(value, collections.Iterable):
	...
	def __next__(self):
		if self.index < len(self):
			rv = _ext.Slice_float32_elem(self.handle, self.index)
			self.index = self.index + 1
			return rv
		raise StopIteration
	def append(self, value):
		_ext.Slice_float32_append(self.handle, value)
	def copy(self, src):
		""" copy emulates the go copy function, copying elements into this list from source list, up to min of size of each list """
		mx = min(len(self), len(src))
		for i in range(mx):
...
class Slice_float64(GoClass):
	...
	def __init__(self, *args, **kwargs):
		...
		if len(kwargs) == 1 and 'handle' in kwargs:
			...
			self.handle = _ext.Slice_float64_CTor()
			_ext.IncRef(self.handle)
			if len(args) > 0:
				if not isinstance(args[0], collections.Iterable):
	...
	def __len__(self):
		return _ext.Slice_float64_len(self.handle)
	def __getitem__(self, key):
		if isinstance(key, slice):
			return [self[ii] for ii in range(*key.indices(len(self)))]
		elif isinstance(key, int):
			...
			if key < 0 or key >= len(self):
				raise IndexError('slice index out of range')
			return _ext.Slice_float64_elem(self.handle, key)
		else:
			raise TypeError('slice index invalid type')
	def __setitem__(self, idx, value):
		if idx < 0:
			idx += len(self)
		if idx < len(self):
			_ext.Slice_float64_set(self.handle, idx, value)
			return
		raise IndexError('slice index out of range')
	def __iadd__(self, value):
		if not isinstance(value, collections.Iterable):
	...
	def __next__(self):
		if self.index < len(self):
			rv = _ext.Slice_float64_elem(self.handle, self.index)
			self.index = self.index + 1
			return rv
		raise StopIteration
	def append(self, value):
		_ext.Slice_float64_append(self.handle, value)
	def copy(self, src):
		""" copy emulates the go copy function, copying elements into this list from source list, up to min of size of each list """
		mx = min(len(self), len(src))
		for i in range(mx):
...
class Slice_int(GoClass):
	...
	def __init__(self, *args, **kwargs):
		...
		if len(kwargs) == 1 and 'handle' in kwargs:
			...
			self.handle = _ext.Slice_int_CTor()
			_ext.IncRef(self.handle)
			if len(args) > 0:
				if not isinstance(args[0], collections.Iterable):
	...
	def __len__(self):
		return _ext.Slice_int_len(self.handle)
	def __getitem__(self, key):
		if isinstance(key, slice):
			return [self[ii] for ii in range(*key.indices(len(self)))]
		elif isinstance(key, int):
			...
			if key < 0 or key >= len(self):
				raise IndexError('slice index out of range')
			return _ext.Slice_int_elem(self.handle, key)
		else:
			raise TypeError('slice index invalid type')
	def __setitem__(self, idx, value):
		if idx < 0:
			idx += len(self)
		if idx < len(self):
			_ext.Slice_int_set(self.handle, idx, value)
			return
		raise IndexError('slice index out of range')
	def __iadd__(self, value):
		if not isinstance(value, collections.Iterable):
	...
	def __next__(self):
		if self.index < len(self):
			rv = _ext.Slice_int_elem(self.handle, self.index)
			self.index = self.index + 1
			return rv
		raise StopIteration
	def append(self, value):
		_ext.Slice_int_append(self.handle, value)
	def copy(self, src):
		""" copy emulates the go copy function, copying elements into this list from source list, up to min of size of each list """
		mx = min(len(self), len(src))
		for i in range(mx):
...
class Slice_int16(GoClass):
	...
	def __init__(self, *args, **kwargs):
		...
		if len(kwargs) == 1 and 'handle' in kwargs:
			...
			self.handle = _ext.Slice_int16_CTor()
			_ext.IncRef(self.handle)
			if len(args) > 0:
				if not isinstance(args[0], collections.Iterable):
	...
	def __len__(self):
		return _ext.Slice_int16_len(self.handle)
	def __getitem__(self, key):
		if isinstance(key, slice):
			return [self[ii] for ii in range(*key.indices(len(self)))]
		elif isinstance(key, int):
			...
			if key < 0 or key >= len(self):
				raise IndexError('slice index out of range')
			return _ext.Slice_int16_elem(self.handle, key)
		else:
			raise TypeError('slice index invalid type')
	def __setitem__(self, idx, value):
		if idx < 0:
			idx += len(self)
		if idx < len(self):
			_ext.Slice_int16_set(self.handle, idx, value)
			return
		raise IndexError('slice index out of range')
	def __iadd__(self, value):
		if not isinstance(value, collections.Iterable):
	...
	def __next__(self):
		if self.index < len(self):
			rv = _ext.Slice_int16_elem(self.handle, self.index)
			self.index = self.index + 1
			return rv
		raise StopIteration
	def append(self, value):
		_ext.Slice_int16_append(self.handle, value)
	def copy(self, src):
		""" copy emulates the go copy function, copying elements into this list from source list, up to min of size of each list """
		mx = min(len(self), len(src))
		for i in range(mx):
...
class Slice_int32(GoClass):
	...
	def __init__(self, *args, **kwargs):
		...
		if len(kwargs) == 1 and 'handle' in kwargs:
			...
			self.handle = _ext.Slice_int32_CTor()
			_ext.IncRef(self.handle)
			if len(args) > 0:
				if not isinstance(args[0], collections.Iterable):
	...
	def __len__(self):
		return _ext.Slice_int32_len(self.handle)
	def __getitem__(self, key):
		if isinstance(key, slice):
			return [self[ii] for ii in range(*key.indices(len(self)))]
		elif isinstance(key, int):
			...
			if key < 0 or key >= len(self):
				raise IndexError('slice index out of range')
			return _ext.Slice_int32_elem(self.handle, key)
		else:
			raise TypeError('slice index invalid type')
	def __setitem__(self, idx, value):
		if idx < 0:
			idx += len(self)
		if idx < len(self):
			_ext.Slice_int32_set(self.handle, idx, value)
			return
		raise IndexError('slice index out of range')
	def __iadd__(self, value):
		if not isinstance(value, collections.Iterable):
	...
	def __next__(self):
		if self.index < len(self):
			rv = _ext.Slice_int32_elem(self.handle, self.index)
			self.index = self.index + 1
			return rv
		raise StopIteration
	def append(self, value):
		_ext.Slice_int32_append(self.handle, value)
	def copy(self, src):
		""" copy emulates the go copy function, copying elements into this list from source list, up to min of size of each list """
		mx = min(len(self), len(src))
		for i in range(mx):
...
class Slice_int64(GoClass):
	...
	def __init__(self, *args, **kwargs):
		...
		if len(kwargs) == 1 and 'handle' in kwargs:
			...
			self.handle = _ext.Slice_int64_CTor()
			_ext.IncRef(self.handle)
			if len(args) > 0:
				if not isinstance(args[0], collections.Iterable):
	...
	def __len__(self):
		return _ext.Slice_int64_len(self.handle)
	def __getitem__(self, key):
		if isinstance(key, slice):
			return [self[ii] for ii in range(*key.indices(len(self)))]
		elif isinstance(key, int):
			...
			if key < 0 or key >= len(self):
				raise IndexError('slice index out of range')
			return _ext.Slice_int64_elem(self.handle, key)
		else:
			raise TypeError('slice index invalid type')
	def __setitem__(self, idx, value):
		if idx < 0:
			idx += len(self)
		if idx < len(self):
			_ext.Slice_int64_set(self.handle, idx, value)
			return
		raise IndexError('slice index out of range')
	def __iadd__(self, value):
		if not isinstance(value, collections.Iterable):
	...
	def __next__(self):
		if self.index < len(self):
			rv = _ext.Slice_int64_elem(self.handle, self.index)
			self.index = self.index + 1
			return rv
		raise StopIteration
	def append(self, value):
		_ext.Slice_int64_append(self.handle, value)
	def copy(self, src):
		""" copy emulates the go copy function, copying elements into this list from source list, up to min of size of each list """
		mx = min(len(self), len(src))
		for i in range(mx):
...
class Slice_int8(GoClass):
	...
	def __init__(self, *args, **kwargs):
		...
		if len(kwargs) == 1 and 'handle' in kwargs:
			...
			self.handle = _ext.Slice_int8_CTor()
			_ext.IncRef(self.handle)
			if len(args) > 0:
				if not isinstance(args[0], collections.Iterable):
	...
	def __len__(self):
		return _ext.Slice_int8_len(self.handle)
	def __getitem__(self, key):
		if isinstance(key, slice):
			return [self[ii] for ii in range(*key.indices(len(self)))]
		elif isinstance(key, int):
			...
			if key < 0 or key >= len(self):
				raise IndexError('slice index out of range')
			return _ext.Slice_int8_elem(self.handle, key)
		else:
			raise TypeError('slice index invalid type')
	def __setitem__(self, idx, value):
		if idx < 0:
			idx += len(self)
		if idx < len(self):
			_ext.Slice_int8_set(self.handle, idx, value)
			return
		raise IndexError('slice index out of range')
	def __iadd__(self, value):
		if not isinstance(value, collections.Iterable):
	...
	def __next__(self):
		if self.index < len(self):
			rv = _ext.Slice_int8_elem(self.handle, self.index)
			self.index = self.index + 1
			return rv
		raise StopIteration
	def append(self, value):
		_ext.Slice_int8_append(self.handle, value)
	def copy(self, src):
		""" copy emulates the go copy function, copying elements into this list from source list, up to min of size of each list """
		mx = min(len(self), len(src))
		for i in range(mx):
...
class Slice_rune(GoClass):
	...
	def __init__(self, *args, **kwargs):
		...
		if len(kwargs) == 1 and 'handle' in kwargs:
			...
			self.handle = _ext.Slice_rune_CTor()
			_ext.IncRef(self.handle)
			if len(args) > 0:
				if not isinstance(args[0], collections.Iterable):
	...
	def __len__(self):
		return _ext.Slice_rune_len(self.handle)
	def __getitem__(self, key):
		if isinstance(key, slice):
			return [self[ii] for ii in range(*key.indices(len(self)))]
		elif isinstance(key, int):
			...
			if key < 0 or key >= len(self):
				raise IndexError('slice index out of range')
			return _ext.Slice_rune_elem(self.handle, key)
		else:
			raise TypeError('slice index invalid type')
	def __setitem__(self, idx, value):
		if idx < 0:
			idx += len(self)
		if idx < len(self):
			_ext.Slice_rune_set(self.handle, idx, value)
			return
		raise IndexError('slice index out of range')
	def __iadd__(self, value):
		if not isinstance(value, collections.Iterable):
	...
	def __next__(self):
		if self.index < len(self):
			rv = _ext.Slice_rune_elem(self.handle, self.index)
			self.index = self.index + 1
			return rv
		raise StopIteration
	def append(self, value):
		_ext.Slice_rune_append(self.handle, value)
	def copy(self, src):
		""" copy emulates the go copy function, copying elements into this list from source list, up to min of size of each list """
		mx = min(len(self), len(src))
		for i in range(mx):
...
class Slice_string(GoClass):
	...
	def __init__(self, *args, **kwargs):
		...
		if len(kwargs) == 1 and 'handle' in kwargs:
			...
			self.handle = _ext.Slice_string_CTor()
			_ext.IncRef(self.handle)
			if len(args) > 0:
				if not isinstance(args[0], collections.Iterable):
	...
	def __len__(self):
		return _ext.Slice_string_len(self.handle)
	def __getitem__(self, key):
		if isinstance(key, slice):
			return [self[ii] for ii in range(*key.indices(len(self)))]
		elif isinstance(key, int):
			...
			if key < 0 or key >= len(self):
				raise IndexError('slice index out of range')
			return _ext.Slice_string_elem(self.handle, key)
		else:
			raise TypeError('slice index invalid type')
	def __setitem__(self, idx, value):
		if idx < 0:
			idx += len(self)
		if idx < len(self):
			_ext.Slice_string_set(self.handle, idx, value)
			return
		raise IndexError('slice index out of range')
	def __iadd__(self, value):
		if not isinstance(value, collections.Iterable):
	...
	def __next__(self):
		if self.index < len(self):
			rv = _ext.Slice_string_elem(self.handle, self.index)
			self.index = self.index + 1
			return rv
		raise StopIteration
	def append(self, value):
		_ext.Slice_string_append(self.handle, value)
	def copy(self, src):
		""" copy emulates the go copy function, copying elements into this list from source list, up to min of size of each list """
		mx = min(len(self), len(src))
		for i in range(mx):
...
class Slice_uint(GoClass):
	...
	def __init__(self, *args, **kwargs):
		...
		if len(kwargs) == 1 and 'handle' in kwargs:
			...
			self.handle = _ext.Slice_uint_CTor()
			_ext.IncRef(self.handle)
			if len(args) > 0:
				if not isinstance(args[0], collections.Iterable):
	...
	def __len__(self):
		return _ext.Slice_uint_len(self.handle)
	def __getitem__(self, key):
		if isinstance(key, slice):
			return [self[ii] for ii in range(*key.indices(len(self)))]
		elif isinstance(key, int):
			...
			if key < 0 or key >= len(self):
				raise IndexError('slice index out of range')
			return _ext.Slice_uint_elem(self.handle, key)
		else:
			raise TypeError('slice index invalid type')
	def __setitem__(self, idx, value):
		if idx < 0:
			idx += len(self)
		if idx < len(self):
			_ext.Slice_uint_set(self.handle, idx, value)
			return
		raise IndexError('slice index out of range')
	def __iadd__(self, value):
		if not isinstance(value, collections.Iterable):
	...
	def __next__(self):
		if self.index < len(self):
			rv = _ext.Slice_uint_elem(self.handle, self.index)
			self.index = self.index + 1
			return rv
		raise StopIteration
	def append(self, value):
		_ext.Slice_uint_append(self.handle, value)
	def copy(self, src):
		""" copy emulates the go copy function, copying elements into this list from source list, up to min of size of each list """
		mx = min(len(self), len(src))
		for i in range(mx):
...
class Slice_uint16(GoClass):
	...
	def __init__(self, *args, **kwargs):
		...
		if len(kwargs) == 1 and 'handle' in kwargs:
			...
			self.handle = _ext.Slice_uint16_CTor()
			_ext.IncRef(self.handle)
			if len(args) > 0:
				if not isinstance(args[0], collections.Iterable):
	...
	def __len__(self):
		return _ext.Slice_uint16_len(self.handle)
	def __getitem__(self, key):
		if isinstance(key, slice):
			return [self[ii] for ii in range(*key.indices(len(self)))]
		elif isinstance(key, int):
			...
			if key < 0 or key >= len(self):
				raise IndexError('slice index out of range')
			return _ext.Slice_uint16_elem(self.handle, key)
		else:
			raise TypeError('slice index invalid type')
	def __setitem__(self, idx, value):
		if idx < 0:
			idx += len(self)
		if idx < len(self):
			_ext.Slice_uint16_set(self.handle, idx, value)
			return
		raise IndexError('slice index out of range')
	def __iadd__(self, value):
		if not isinstance(value, collections.Iterable):
	...
	def __next__(self):
		if self.index < len(self):
			rv = _ext.Slice_uint16_elem(self.handle, self.index)
			self.index = self.index + 1
			return rv
		raise StopIteration
	def append(self, value):
		_ext.Slice_uint16_append(self.handle, value)
	def copy(self, src):
		""" copy emulates the go copy function, copying elements into this list from source list, up to min of size of each list """
		mx = min(len(self), len(src))
		for i in range(mx):
...
class Slice_uint32(GoClass):
	...
	def __init__(self, *args, **kwargs):
		...
		if len(kwargs) == 1 and 'handle' in kwargs:
			...
			self.handle = _ext.Slice_uint32_CTor()
			_ext.IncRef(self.handle)
			if len(args) > 0:
				if not isinstance(args[0], collections.Iterable):
	...
	def __len__(self):
		return _ext.Slice_uint32_len(self.handle)
	def __getitem__(self, key):
		if isinstance(key, slice):
			return [self[ii] for ii in range(*key.indices(len(self)))]
		elif isinstance(key, int):
			...
			if key < 0 or key >= len(self):
				raise IndexError('slice index out of range')
			return _ext.Slice_uint32_elem(self.handle, key)
		else:
			raise TypeError('slice index invalid type')
	def __setitem__(self, idx, value):
		if idx < 0:
			idx += len(self)
		if idx < len(self):
			_ext.Slice_uint32_set(self.handle, idx, value)
			return
		raise IndexError('slice index out of range')
	def __iadd__(self, value):
		if not isinstance(value, collections.Iterable):
	...
	def __next__(self):
		if self.index < len(self):
			rv = _ext.Slice_uint32_elem(self.handle, self.index)
			self.index = self.index + 1
			return rv
		raise StopIteration
	def append(self, value):
		_ext.Slice_uint32_append(self.handle, value)
	def copy(self, src):
		""" copy emulates the go copy function, copying elements into this list from source list, up to min of size of each list """
		mx = min(len(self), len(src))
		for i in range(mx):
...
class Slice_uint64(GoClass):
	...
	def __init__(self, *args, **kwargs):
		...
		if len(kwargs) == 1 and 'handle' in kwargs:
			...
			self.handle = _ext.Slice_uint64_CTor()
			_ext.IncRef(self.handle)
			if len(args) > 0:
				if not isinstance(args[0], collections.Iterable):
	...
	def __len__(self):
		return _ext.Slice_uint64_len(self.handle)
	def __getitem__(self, key):
		if isinstance(key, slice):
			return [self[ii] for ii in range(*key.indices(len(self)))]
		elif isinstance(key, int):
			...
			if key < 0 or key >= len(self):
				raise IndexError('slice index out of range')
			return _ext.Slice_uint64_elem(self.handle, key)
		else:
			raise TypeError('slice index invalid type')
	def __setitem__(self, idx, value):
		if idx < 0:
			idx += len(self)
		if idx < len(self):
			_ext.Slice_uint64_set(self.handle, idx, value)
			return
		raise IndexError('slice index out of range')
	def __iadd__(self, value):
		if not isinstance(value, collections.Iterable):
	...
	def __next__(self):
		if self.index < len(self):
			rv = _ext.Slice_uint64_elem(self.handle, self.index)
			self.index = self.index + 1
			return rv
		raise StopIteration
	def append(self, value):
		_ext.Slice_uint64_append(self.handle, value)
	def copy(self, src):
		""" copy emulates the go copy function, copying elements into this list from source list, up to min of size of each list """
		mx = min(len(self), len(src))
		for i in range(mx):
...
class Slice_uint8(GoClass):
	...
	def __init__(self, *args, **kwargs):
		...
		if len(kwargs) == 1 and 'handle' in kwargs:
			...
			self.handle = _ext.Slice_uint8_CTor()
			_ext.IncRef(self.handle)
			if len(args) > 0:
				if not isinstance(args[0], collections.Iterable):
	...
	def __len__(self):
		return _ext.Slice_uint8_len(self.handle)
	def __getitem__(self, key):
		if isinstance(key, slice):
			return [self[ii] for ii in range(*key.indices(len(self)))]
		elif isinstance(key, int):
			...
			if key < 0 or key >= len(self):
				raise IndexError('slice index out of range')
			return _ext.Slice_uint8_elem(self.handle, key)
		else:
			raise TypeError('slice index invalid type')
	def __setitem__(self, idx, value):
		if idx < 0:
			idx += len(self)
		if idx < len(self):
			_ext.Slice_uint8_set(self.handle, idx, value)
			return
		raise IndexError('slice index out of range')
	def __iadd__(self, value):
		if not isinstance(value, collections.Iterable):
	...
	def __next__(self):
		if self.index < len(self):
			rv = _ext.Slice_uint8_elem(self.handle, self.index)
			self.index = self.index + 1
			return rv
		raise StopIteration
	def append(self, value):
		_ext.Slice_uint8_append(self.handle, value)
	def copy(self, src):
		""" copy emulates the go copy function, copying elements into this list from source list, up to min of size of each list """
		mx = min(len(self), len(src))
		for i in range(mx):
